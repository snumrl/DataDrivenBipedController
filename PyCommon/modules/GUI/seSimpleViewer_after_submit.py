# +-------------------------------------------------------------------------
# | seSimpleViewer_after_submit.py
# |
# | Author: Yoonsang Lee
# +-------------------------------------------------------------------------
# | COPYRIGHT:
...
import GUI.seViewer3_after_submit as yv3
...
class SliderWnd(Fl_Hor_Slider):
   
    def __init__(self,x,y,w,h,l):
        Fl_Hor_Slider.__init__(self,x,y,w,h,l)
       
    def handle(self,event):
        if event == FL_PUSH: 
            self.val_slider_handle('on')
        elif event == FL_RELEASE :
            self.val_slider_handle('off')
        return Fl_Hor_Slider.handle(self, event)
...
#        self.box(FL_PLASTIC_UP_BOX)
        ...
        self.direction_0 = Fl_Button(70-but_width,60-but_height,but_width,but_height,'45_L')
        self.direction_0.callback(self.but_callback,45)
        self.direction_1 = Fl_Button(70,60-but_height,but_width,but_height,'N')
        self.direction_1.callback(self.but_callback,0)
        self.direction_2 = Fl_Button(70+but_width,60-but_height,but_width,but_height,'45_R')
        self.direction_2.callback(self.but_callback,315)
        self.direction_3 = Fl_Button(70-but_width,60,but_width,but_height,'E')
        self.direction_3.callback(self.but_callback,90)
        self.direction_4 = Fl_Button(70+but_width,60,but_width,but_height,'W')
        self.direction_4.callback(self.but_callback,270)
        self.direction_5 = Fl_Button(70-but_width,60+but_height,but_width,but_height,'135_L')
        self.direction_5.callback(self.but_callback,135)
        self.direction_6 = Fl_Button(70,60+but_height,but_width,but_height,'S')
        self.direction_6.callback(self.but_callback,180)
        self.direction_7 = Fl_Button(70+but_width,60+but_height,but_width,but_height,'135_R')
        self.direction_7.callback(self.but_callback,225)
        
        self.angle_value_input = Fl_Value_Input(0,60+but_height*3, 30, 20)
        self.angle_value_input.step(.01)
        self.angle_value_input.callback(self.angle_value_input_cb)
        
#        self.angle_value_slider = Fl_Hor_Slider(70-but_width,60+but_height*3,150, 20,'angle')   
        self.angle_value_slider = SliderWnd(70-but_width,60+but_height*3,150, 20,'angle')
        self.angle_value_slider.bounds(180, -180)
        self.angle_value_slider.value(0) 
        
        self.angle_value_slider.callback(self.angle_value_slider_cb)
        
        
        self.speed_value_input = Fl_Value_Input(0,60+but_height*4+10, 30, 20)
        self.speed_value_input.step(.01)
        self.speed_value_input.callback(self.speed_value_input_cb)
        
        self.speed_value_slider = Fl_Hor_Slider(70-but_width,60+but_height*4+10,150, 20,'speed')
        self.speed_value_slider.bounds(0, 10)
        self.speed_value_slider.value(0)
        
        self.speed_value_slider.callback(self.speed_value_slider_cb)
        
#        self.angle_value.callback(self.onChangeSlider)
        self.style0_but = Fl_Button(0, 60+but_height*5+20, 40,20, 'Normal')
        self.style0_but.callback(self.style_but_callback,0)
#        self.style1_but = Fl_Button(42, 60+but_height*5+20, 40,20, 'Style1')
#        self.style1_but.callback(self.style_but_callback,1)
#        self.style1_but = Fl_Button(42*2, 60+but_height*5+20, 40,20, 'apply')
#        self.style1_but.callback(self.style_but_callback,2)
        self.style1_but = Fl_Button(42*2, 60+but_height*5+20, 40,20, 'Style1')
        self.style1_but.callback(self.style_but_callback,1)
        self.style1_but = Fl_Button(42*3, 60+but_height*5+20, 40,20, 'Style3')
        self.style1_but.callback(self.style_but_callback,3)
...
#        return Fl_Window.handle(self, event)
    ...
    def speed_value_slider_cb(self,ptr):
        self.speed_value_input.value(int(ptr.value()))
#        self.speed_but_cb(int(ptr.value()))
        self.speed_but_cb(float(ptr.value()))
        
    def speed_value_input_cb(self,ptr):
        self.speed_value_slider.value(int(ptr.value()))
#        self.speed_but_cb(int(ptr.value()))
    ...
    def angle_value_slider_cb(self,ptr):
#        self.angle_value_input.value(int(ptr.value()))
        self.angle_value_input.value(float(ptr.value()))
        self.angle_but_cb_handle(float(ptr.value()) )
        
#        if int(ptr.value()) < 0:
#            self.angle_but_cb(float(360+ptr.value()))
#        else:
#            self.angle_but_cb(float(ptr.value()))

            
#        if int(ptr.value()) == -180:
#            self.angle_value_input.value(180)
#        if int(ptr.value()) == 180:
#            self.angle_value_input.value(-180)     
    def angle_value_input_cb(self,ptr):
        self.angle_value_slider.value(float(ptr.value()))
#        if int(ptr.value()) < 0:
#            self.angle_but_cb(int(360+ptr.value())) 
#        else:
#            self.angle_but_cb(int(ptr.value()))
        
    def style_but_callback(self,ptr,which):
        self.style_but_cb(which)
    def but_callback(self,ptr,which):
        
        self.angle_but_cb(which)
...
#        viewer.startTimer(1/pointMotion.fps)
    ...
    def test_Fl_Check_Browser():
        def cb_callback(ptr,which):
            print which
            
        win = Fl_Window(100,100,200,200)
        win.begin()
#        cb = Fl_Check_Browser(100,100,100,100)
        cb = Fl_Button (100,100,50,50,'0')
        cb.callback(cb_callback,'0')
#        cb.add('test1', 1)
#        cb.add('test2', 0)
        win.end()
...
#            time.sleep(0.03)
    ...
    def test_slider():
        ...
        test = Fl_Hor_Slider(0,10,100,10,'a')
        test3 = SliderWnd(-10,0,120,10,'d')
        test2 = SliderWnd(50,30,60,10,'d')
