...
public:	// expose to python
	VpBody(VpWorld* pWorld);
	void addBoxGeom(const object& size, scalar density);
	object getShape();
	void setFrame(const object& pyT);
	object getFrame();
	void setPosition( const object& pyV );
	void setOrientation( const object& pyR );
};
