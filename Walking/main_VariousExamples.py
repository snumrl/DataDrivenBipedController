...
#    motion_ori.translateByOffset((0, boxGroundHeight, 0))

    #===============================================================================
    # boxes creation
    #===============================================================================
    class Box:
        def __init__(self, size, pos, ori):
            self.size = size
            self.pos = pos
            self.ori = ori
        def __str__(self):
            # right : +x direction, bottom : +z direction
            s = ''
            s += 'lb : ' + self.getPosition('lb').__str__() + '\n'
            s += 'rb : ' + self.getPosition('rb').__str__() + '\n'
            s += 'rt : ' + self.getPosition('rt').__str__() + '\n'
            s += 'lt : ' + self.getPosition('lt').__str__() + '\n'
            return s
        def getPosition(self, cornerName):
            if cornerName=='lb':
                localPoint = (-self.size[0]/2., self.size[1]/2., self.size[2]/2.)
            elif cornerName=='rb':
                localPoint = (self.size[0]/2., self.size[1]/2., self.size[2]/2.)
            elif cornerName=='rt':
                localPoint = (self.size[0]/2., self.size[1]/2., -self.size[2]/2.)
                ...
                localPoint = (-self.size[0]/2., self.size[1]/2., 0.)
            elif cornerName=='rc':
                localPoint = (self.size[0]/2., self.size[1]/2., 0.)
            elif cornerName=='tc':
                localPoint = (0., self.size[1]/2., -self.size[2]/2.)
            elif cornerName=='bc':
                localPoint = (0., self.size[1]/2., self.size[2]/2.)
            else: print 'ERROR'; return None
            return np.dot(self.ori, localPoint) + self.pos
        def translateByTarget(self, p_target, cornerName):
            p_offset = p_target - self.getPosition(cornerName)
            self.pos += p_offset
            
    boxThick = .01
    boxWidth = 2.
    
    start_box = Box( (2, boxThick, boxWidth), (0, height, 0), mm.I_SO3() )
    slope_box = Box( (20, boxThick, boxWidth), (0, 0, 0), mm.rotZ(-slope_deg*mm.RAD) )
    
    motion_pos = motion_ori[0].rootPos.copy(); motion_pos[1]=height; motion_pos[0]+=.5
    motion_ori.translateByOffset((0,height,0))
    start_box.translateByTarget(motion_pos, 'rc')
    slope_box.translateByTarget(start_box.getPosition('lc'), 'rc')
    
    boxes = [start_box, slope_box]
    
    rd_boxes =  yr.BoxesRenderer([b.size for b in boxes], [mm.Rp2T(b.ori, b.pos) for b in boxes], BOX_COLOR)
...
#    stf_push_func = [yfg.zero]
    ...
    rd_contactForces = [None]; rd_contactPositions = [None]
...
#        viewer = ymv.MultiViewer(1600, 1255)
        ...
        viewer.setRenderers2([cvr.VpModelRenderer(controlModel, CHARACTER_COLOR, yr.POLYGON_FILL), rd_boxes])
    else:
...
#        viewer.doc.addRenderer('motionModel', cvr.VpModelRenderer(motionModel, (0,150,255), yr.POLYGON_LINE))
        ...
        rd_boxes.rc.setPolygonStyle(yr.POLYGON_LINE)
        viewer.doc.addRenderer('rd_boxes', rd_boxes)
...
#        viewer.doc.addRenderer('contactForces', yr.VectorsRenderer(rd_contactForces, rd_contactPositions, (0,255,0)))
...
#            vpWorld.applyPenaltyForce(bodyIDs, contactPositionLocals, contactForces)
            ...
            bodyIDs, contactPositions, contactPositionLocals, contactForces = vpWorld.calcPenaltyForce_Boxes(rd_boxes.boxSizes, rd_boxes.Ts, bodyIDsToCheck, mus, Ks, Ds)
            vpWorld.applyPenaltyForce(bodyIDs, contactPositionLocals, contactForces)

#            bodyIDs1, contactPositions1, contactPositionLocals1, contactForces1 = vpWorld.calcPenaltyForce(bodyIDsToCheck, mus, Ks, Ds)
#            bodyIDs2, contactPositions2, contactPositionLocals2, contactForces2 = vpWorld.calcPenaltyForce_Boxes(rd_boxes.boxSizes, rd_boxes.Ts, bodyIDsToCheck, mus, Ks, Ds)
#            bodyIDs = bodyIDs1 + bodyIDs2
#            contactPositions = contactPositions1 + contactPositions2
#            contactPositionLocals = contactPositionLocals1 + contactPositionLocals2
#            contactForces = contactForces1 + contactForces2
...
#        F /= stepsPerFrame
        ...
        rd_contactPositions[:] = contactPositions
        rd_contactForces[:] = contactForces
...
#    # disable feedback on stance hips
#    SEGMENT_EDITING =           True
#    STANCE_FOOT_STABILIZE =     True
#    MATCH_STANCE_LEG =          False
#    SWING_FOOT_PLACEMENT =      True
#    SWING_FOOT_HEIGHT =         True
#    
#    SWING_FOOT_ORIENTATION =    False
...
#    # disable feedback on swing hips
#    SEGMENT_EDITING =           True
#    STANCE_FOOT_STABILIZE =     True
#    MATCH_STANCE_LEG =          True
#    SWING_FOOT_PLACEMENT =      False
#    SWING_FOOT_HEIGHT =         True
#    
#    SWING_FOOT_ORIENTATION =    False
#    
...
#    # disable feedback on stance ankles
#    SEGMENT_EDITING =           True
#    STANCE_FOOT_STABILIZE =     True
#    MATCH_STANCE_LEG =          True
#    SWING_FOOT_PLACEMENT =      True
...
#    STANCE_FOOT_BALANCING =     False
    ...
    # disable feedback for swing foot height
    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = .3; K_swp_pos_cor = 0.
    K_stp_pos = 1.
#    filename = 'wd2_WalkForwardSlow01.bvh'
    filename = 'wd2_WalkForwardSlow01_REPEATED.bvh' # 3 frame diff
    ...
    motion_ori = yf.readBvhFile(dir+filename)
    motion_saved = yf.readBvhFile(SAVE_DIR + 'simulated_'  + filename)
    frameTime = 1/motion_ori.fps
...
##    forceInfos = [ForceInfo(4*i*30+1, .4, (-130,0,0)) for i in range(2,12)]
...
##    mu = 1.
##    c_swf_mid_offset = .0
##    K_swp_vel_sag = .4; K_swp_vel_sag_faster = .0;  
##    K_swp_pos_sag = .1; K_swp_pos_sag_faster = .0;
##    K_swp_vel_cor = .3;  K_swp_pos_cor = .2
##    K_stp_pos = 0.
##    K_stb_vel = .1
##    K_stb_pos = .0
##    filename = 'wd2_WalkForwardSlow01_REPEATED_FOOT.bvh'
#
##    Kt = 40.;       Dt = 2*(Kt**.5)
##    Ks = 3000.;    Ds = 2*(Ks**.5)
##    mu = 1.
...
##    K_stb_vel = .02
##    K_stb_pos = .15
##    filename = 'wd2_WalkForwardSlow01_REPEATED.bvh'
...
#    c_swf_mid_offset = .0
    ...
    # compare natural foot motion & fixed foot motion
    Kt = 40.;       Dt = 2*(Kt**.5)
    Ks = 3000.;    Ds = 2*(Ks**.5)
    mu = 2.
    c_swf_mid_offset = .0
    K_swp_vel_sag = .0; K_swp_vel_sag_faster = .0;  
    K_swp_pos_sag = .5; K_swp_pos_sag_faster = .0;
    K_swp_vel_cor = .3;  K_swp_pos_cor = .3
    K_stp_pos = 0.
    K_stb_vel = .1
    K_stb_pos = .0
...
#    forceInfos = [ForceInfo(4*i*30, .4, (35,0,0)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30+1, .4, (-100,0,0)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30+1, .4, (0,0,25)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30, .4, (0,0,-45)) for i in range(2,12)]
#    filename = 'wd2_WalkSoldier00_REPEATED.bvh'

#    forceInfos = [ForceInfo(4*i*30, .4, (40,0,0)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30+1, .4, (-110,0,0)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30+1, .4, (0,0,40)) for i in range(2,12)]
#    forceInfos = [ForceInfo(4*i*30, .4, (0,0,-55)) for i in range(2,12)]
    filename = 'wd2_WalkSoldier00_REPEATED_FOOT.bvh'

    forceInfos = [ForceInfo(4*i*30+1, .4, (80,0,0)) for i in range(2,12)]
...
#    mcfgfile = open(dir + 'mcfg', 'r')
#    mcfgfile = open('mcfg_simbicon', 'r')
    mcfgfile = open('mcfg_simbicon_fixed_foot', 'r')
    mcfg = cPickle.load(mcfgfile)
    mcfgfile.close()
...
#    K_stb_vel = .2
    K_stp_pos = 1.
#    filename = 'wd2_WalkSameSame01.bvh'
    filename = 'wd2_WalkSameSame01_REPEATED.bvh'    # 5 min

#    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = .3; K_swp_pos_cor = 0.
#    K_stp_pos = 1.
##    filename = 'wd2_WalkForwardSlow01.bvh'
#    filename = 'wd2_WalkForwardSlow01_REPEATED.bvh' # 5 min

#    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = 1.; K_swp_pos_cor = 0.
#    K_stp_pos = .6
##    filename = 'wd2_WalkForwardNormal00.bvh'
#    filename = 'wd2_WalkForwardNormal00_REPEATED.bvh'   # 5 min

#    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = .3; K_swp_pos_cor = 0.
#    K_stp_pos = 0.
##    filename = 'wd2_WalkHandWav00.bvh'
#    filename = 'wd2_WalkHandWav00_REPEATED.bvh' # 5 min
...
#    K_stp_pos = 0.
##    filename = 'wd2_WalkAzuma01.bvh'
#    filename = 'wd2_WalkAzuma01_REPEATED.bvh'   # 5 min

#    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = 1.; K_swp_pos_cor = 0.
#    K_stp_pos = 0.
##    filename = 'wd2_WalkSoldier00.bvh'    # K_swp_pos_sag = .0
#    filename = 'wd2_WalkSoldier00_REPEATED.bvh'    # 5 min

#    mu = 2.
#    K_swp_vel_sag = .2; K_swp_vel_cor = .4; K_swp_pos_sag = .5;K_swp_pos_cor = 0.
#    K_stp_pos = 0.
##    filename = 'wd2_WalkForwardVFast00.bvh'
#    filename = 'wd2_WalkForwardVFast00_REPEATED.bvh'    # 5 min
#
...
###    K_swp_vel_sag = .1; K_swp_vel_cor = .4; K_swp_pos_sag = 1.4; K_swp_pos_cor = 0.
##    K_swp_vel_sag = .0; K_swp_vel_cor = .25; K_swp_pos_sag = .1; K_swp_pos_cor = .28
##    K_swp_pos_sag_faster = .22
##    K_stp_pos = 0.
###    K_stb_vel = .2
##    K_stb_vel = .2
##    c_swf_mid_offset = .002
###    filename = 'wd2_WalkBackward00.bvh'
##    filename = 'wd2_WalkBackward00_REPEATED.bvh'
...
#    c_min_contact_vel = 2.
    ...
    c_swf_mid_offset = .05
    mu = 2.
#    K_swp_vel_sag = .0; K_swp_vel_cor = .4; K_swp_pos_sag = 1.2; K_swp_pos_cor = .0    
    K_swp_vel_sag = .0; K_swp_vel_cor = .4; K_swp_pos_sag = 1.2; K_swp_pos_cor = .1
    K_stp_pos = 0.
    filename = 'wd2_u-turn_1.bvh'
...
#    K_stp_pos = 0.
#    K_stb_vel = .2
#    filename = 'walk_left_45degree.bvh'
##    filename = 'walk_right_45degree.bvh'

#    c_swf_mid_offset = .05
#    c_swf_stability = .3
#    mu = 2.
#    K_swp_vel_sag = .1; K_swp_vel_cor = .25; K_swp_pos_sag = .0; K_swp_pos_cor = .1
#    K_stp_pos = 0.
#    filename = 'walk_left_90degree.bvh'
...
#                    rd_vecori2[0] = controlModel.getJointPositionGlobal(stanceFoots[0])
                ...
                seg_index[0] += 1
                curSeg = segments[seg_index[0]]
#                print yma.offsetInterval(acc_offset[0], seginfo[seg_index[0]]['interval'])
...
#    dir = './ppmotion/'
    dir = './ppmotion/test_spin_first/'
    

    c_swf_mid_offset = .05
    c_swf_stability = .6
    c_locking_vel = 0.04
    K_swp_vel_sag = .1; K_swp_vel_cor = .2; K_swp_pos_sag = 1.; K_swp_pos_cor = .1
    K_stp_pos = 0.
    K_stb_vel = .2
    mu = 3.
    filename = 'wd2_2foot_walk_turn2.bvh'
...
#            viewer = ymv.MultiViewer(1600, 1255)
            ...
            viewer = ymv.MultiViewer(2560, 2048, wheelWork=True, singleView=True)
        viewer.setRenderers1([cvr.VpModelRenderer(motionModel, MOTION_COLOR, yr.POLYGON_FILL)])
        viewer.setRenderers2([cvr.VpModelRenderer(controlModel, CHARACTER_COLOR, yr.POLYGON_FILL)])
    else:
        viewer = ysv.SimpleViewer()
...
#        if frame==1: pt[0] = time.time()
#        if frame==31: print 'elapsed time for 30 frames:', time.time()-pt[0]
        if CAMERA_TRACKING and not TEASER_IMAGE:
            if MULTI_VIEWER:
                if cameraTargets1[frame]==None:
                    cameraTargets1[frame] = motionModel.getBodyPositionGlobal(0)
#                    cameraTargets1[frame] = motion_ori[frame].getJointPositionGlobal(0)
...
#                motion_stitch.extend(ymb.getStitchedNextMotion(curSeg, motion_control[-1], transitionLength, stitch_func, True, True))
        ...
        if not TEASER_IMAGE:
            ...
            motionModel.update(motion_seg[frame])
            multi_frame_list.append(viewer.glWindow2.getState())
            
            if frame==len(motion_ori)-1:
                viewer.glWindow2.multi_frame_list = [multi_frame_list[i] for i in range(30, len(motion_ori)-20, 25)]
...
#            if frame == 1:
                yf.writeBvhFile(SAVE_DIR+filename, motion_simulation)
...
#disable_each_part()
#push_simbicon_mass()
#push_versus_realistic()
#walkings_long()
#walkings_nonzero_param()
#turnings_nonzero_param()
spinning_nonzero_param()